...
import { OfferDocumentCategory } from "@/features/offers/store/offerDocumentsStore";
import { Button } from "@shared/components/ui/button";
import { X, Upload, RefreshCw, FileText, Image, Download } from "lucide-react";
...
const FileUpload: React.FC<FileUploadProps> = ({
  ...
  iconClassName,
  category,
  accept = ".pdf,.doc,.docx,.txt,.jpg,.jpeg,.png,.webp,.svg, .zip, .rar",
  maxFiles = 10,
  ...
  const { documents, handleFileUpload, handleFileRemoval, handleFileReupload } =
    useCreateOffer();
  ...
  const handleFiles = async (files: File[]) => {
    ...
    if (validFiles.length > 0) {
      handleFileUpload(category, validFiles);
      if (onUpload) {
        onUpload(validFiles);
      }
    }
  ...
  const handleRemove = async (fileId: string) => {
    if (!removingFileIds.has(fileId)) {
      setRemovingFileIds((prev) => new Set(prev).add(fileId));
      try {
        if (onRemove) {
          await onRemove(fileId);
        } else {
          await handleFileRemoval(category, fileId);
        }
      } finally {
  ...
  const handleReuploadSelect = async (
    ...
    if (onReupload) {
      await onReupload(reuploadingFileId, file);
    } else if (reuploadingFileId) {
      await handleFileReupload(category, reuploadingFileId, file);
    }
    ...
    <div className={cn("w-full", className)}>
      ...
      {categoryFiles.length > 0 && (
        <div className="mt-4 space-y-2 ">
          {categoryFiles.map((file: File | string | any) => (
            <div
              key={file.id}
              className={twMerge(
