...
import {
  ...
  Documents,
  ProfessionalInfo,
} from "@/features/profile/components/supplier/overview";
...
const SupplierProfileTabContent: React.FC<SupplierProfileTabContentProps> = ({
  ...
  const { handleSupplierProfileFetch, supplierPersonalInfo, isLoading, error } =
  ...
  const getContentForTab = (tabIndex: number) => {
    ...
    if (tabIndex === 0) {
      const hasData =
        supplierPersonalInfo && Object.keys(supplierPersonalInfo).length > 0;
  ...
  const loadContent = useCallback(async () => {
    try {
      await handleSupplierProfileFetch();
      setHasInitialLoad(true);
    } catch (err: unknown) {
      console.error("Error loading supplier profile content:", err);
      setHasInitialLoad(true);
    }
  ...
  }, [supplierPersonalInfo, activeTab, isLoading, hasInitialLoad]);
