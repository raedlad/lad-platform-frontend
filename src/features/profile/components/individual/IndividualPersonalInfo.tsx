...
import { Phone, Mail, UserRound, Pencil, Check, X } from "lucide-react";
...
export const IndividualPersonalInfo = () => {
  ...
  const IndividualProfilePersonalInfoSchema = useMemo(() => {
  ...
  useEffect(() => {
    const subscription = form.watch((values) => {
      if (values) {
        setIndividualPersonalInfo(values as IndividualProfilePersonalInfo);
      }
    });
    return () => subscription.unsubscribe();
  }, [form.watch, setIndividualPersonalInfo]);
  ...
  return (
    <div className={cn("space-y-4 sm:space-y-6")}>
      <div className="w-full form-container max-w-md sm:min-w-xs">
        <Form {...form}>
          <form
            onSubmit={form.handleSubmit(handleIndividualPersonalInfoSubmit)}
              ...
              render={({ field }) => (
                <FormItem className="space-y-0.5">
                  <div className="flex  items-center gap-2">
                    ...
                    <FormLabel>
                      {t("profile.individual.personalInfo.fullName")}
                    </FormLabel>
                  </div>
                  <FormControl>
                    <Input
              ...
              render={({ field }) => (
                <FormItem className="space-y-0.5">
                  <div className="flex  items-center gap-2">
                    ...
                    <FormLabel>
                      {t("profile.individual.personalInfo.email")}
                    </FormLabel>
                  </div>
              ...
              render={({ field }) => (
                <FormItem className="space-y-0.5">
                  <div className="flex  items-center gap-2">
                    ...
                    <FormLabel>
                      {t("profile.individual.personalInfo.phoneNumber")}
                    </FormLabel>
                  </div>
                  <FormControl>
                    <PhoneInput
