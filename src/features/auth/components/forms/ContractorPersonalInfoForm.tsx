...
import { GetPasswordStrength } from "@auth/utils/getPasswordStrength";
const ContractorPersonalInfoForm: React.FC = () => {
  const store = useAuthStore();
  const { handlePersonalInfoSubmit } = useContractorRegistration();
  ...
  React.useEffect(() => {
    if (authMethod === "thirdParty" && store.roleData.personalInfo) {
      if (store.roleData.personalInfo.companyName) {
        form.setValue("companyName", store.roleData.personalInfo.companyName);
      }
