...
type StepStatus = "locked" | "current" | "completed" | "accessible";
...
interface DynamicToolbarExpandableProps {
  ...
  onStepValidation?: (stepId: string) => boolean;
  restrictNavigation?: boolean;
  onStepAccessDenied?: (stepId: string) => void;
}
...
const DynamicToolbarExpandable = React.memo<DynamicToolbarExpandableProps>(
  function DynamicToolbarExpandable({
    ...
    // Function to determine step status and accessibility
    const getStepStatus = useCallback(
      (stepId: string, index: number): StepStatus => {
        ...
        // In restricted mode, only allow access to:
        // 1. Completed steps
        // 2. The current step
        // 3. The next incomplete step after all previous steps are completed
        ...
        return previousStepsCompleted ? "accessible" : "locked";
      },
      [active, completedSteps, restrictNavigation, steps]
    );
    ...
    const handleNavClick = useCallback(
      (item: string) => {
        const stepIndex = steps.findIndex((step) => step.id === item);

        // Check if step is accessible
        if (!isStepAccessible(item, stepIndex)) {
          onStepAccessDenied?.(item);
          return;
        }

        // Validate step access if validation function is provided
        if (onStepValidation && !onStepValidation(item)) {
          return;
        }
        ...
        if (stepIndex >= 0) {
          setTimeout(() => {
            scrollButtonIntoView(stepIndex, previousIndex);
          }, 100);

          setPreviousIndex(stepIndex);
        }
      },
      [
        active,
    ...
    const navigationButtons = useMemo(
      ...
      [steps, active, handleNavClick, getStepStatus, isStepAccessible]
    );

    return (
      <div
        className={cn(
          "space-y-2 w-full max-w-xs sm:max-w-lg mx-auto px-1 sm:px-0",
          className
        )}
      >
        {badgeText && (
            ...
            <div className="flex items-center p-[1px] w-max min-w-full overflow-x-auto">
              {navigationButtons.map((button, index) => (
                <button
                  ...
                  onClick={button.isAccessible ? button.onClick : undefined}
                  disabled={!button.isAccessible}
                  className={cn(
                    "text-xs sm:text-sm transition-colors py-2 px-2 sm:py-4 sm:px-4 whitespace-nowrap flex items-center gap-1 sm:gap-2 min-h-[40px] sm:min-h-[44px] flex-shrink min-w-0",
                    // Active step
                    button.isActive &&
                    ...
                    button.isAccessible &&
                      !button.isActive &&
                      !button.isCompleted &&
                      "text-muted-foreground hover:bg-muted/60 active:bg-muted/70",
                    // Locked step
                    button.isLocked &&
                      "text-muted-foreground/50 cursor-not-allowed opacity-60"
                  )}
                >
                  <div className="text-[9px] sm:text-[10px] w-4 h-4 sm:w-5 sm:h-5 flex items-center justify-center shadow-[0px_1px_1px_0px_hsla(0,0%,0%,0.02)_inset,0px_1px_1px_0px_hsla(0,0%,0%,0.02)_inset,0px_0px_0px_1px_rgba(255,255,255,0.25)] dark:shadow-[0px_1px_1px_0px_hsla(0,0%,100%,0.02)_inset,0px_1px_1px_0px_rgba(255,255,255,0.05)_inset,0px_0px_0px_1px_hsla(0,0%,100%,0.05)_inset,0px_0px_1px_0px_rgba(0,0,0,0.25)] font-bold rounded-md transition-all duration-300 flex-shrink-0">
                    <span
                      className={cn(
                    ...
                    >
                      {button.isCompleted ? "✓" : button.step}
                    </span>
                  </div>
                  <span
                    className={cn(
                      "text-[10px] sm:text-sm font-medium truncate max-w-[60px] sm:max-w-none",
                      ...
                      button.isAccessible &&
                        ...
                        !button.isCompleted &&
                        "text-foreground",
                      button.isLocked && "text-muted-foreground/50"
                    )}
                    title={button.label} // Show full text on hover
                  >
                    {button.label}
                  </span>
                </button>
